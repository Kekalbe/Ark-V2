...
async function loadOperatorsData() {
    const resp = await fetch('./Rhodes-Island/FutureJSONdata/operatorsData.json');
...
async function loadSkillsData() {
    const resp = await fetch('./Rhodes-Island/FutureJSONdata/skillsData.json');
...
function applyOperatorTransform(isMoved) {
  ...
  const transformVal = isMoved ? getResponsiveValue(operator, 'movedForm')
    						   : getResponsiveValue(operator, 'transform');

  characterImg.style.transition = 'transform 0.5s ease';
  setTimeout(() => { characterImg.style.transform = cleanString(transformVal); } );
  ...
  container.innerHTML = `
    ...
    <div class="operator-selector-image"> 
      <img src="${operator.characterImage}" id="${operator.id}" style="width: ${widthInBox}; transform: ${transformInBox};"     
		alt="Card do operador ${operator.name}" /> 
    </div>
...
const updateSkills = (operatorId) => {
  ...
  skillKeys.forEach((skillKey) => {
    ...
    skillDiv.innerHTML = `
      <figure>
        <img src="${skill.imageSkill}" loading="lazy" alt="Skill do Operador ${operator.name}"/>
      </figure>
...
function getVisibleCount() {
  ...
  // 📱 Portrait (máx 2)
  if (isPortrait()) {
    if (ratio > 0.8 && width >= 360) return 2;
    return 1;
  }

  // 🖥️ Landscape (com ratio)
  if (width >= 1200 && ratio > 1.5) return 4;
  if (width >= 850 && ratio > 1.2 || width < 900 && ratio > 1.7 && height < width * 0.7) return 3;
  return 2;
}
...
window.addEventListener('resize', () => {
  ...
  isResizing = true;
  isDraggingList = false;  // <-- desbloqueia arraste preso
  pageDragIsDragging = false;  
  animating = false;       // <-- evita bloqueio por transições interrompidas
...
function endPageDrag() {
  ...
  pageDragDiffX = 0;
    ...
    setTimeout(() => {
    isDraggingList = false;
  }, 50); // pequena margem para evitar falso-positivo
}
